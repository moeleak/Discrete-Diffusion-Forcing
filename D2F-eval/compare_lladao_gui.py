...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-generation-speedup", type=float, default=1.25)
    parser.add_argument("--min-end-to-end-speedup", type=float, default=1.0)
...
def score(path: Path, benchmark: str) -> float:
    result = json.loads(path.read_text(encoding="utf-8"))
    return 100.0 * float(result["benchmarks"][benchmark]["ssr_point_only"])
...
def main() -> None:
    ...
    baseline_ssr = score(args.baseline_scores, args.benchmark)
    d2f_ssr = score(args.d2f_scores, args.benchmark)
