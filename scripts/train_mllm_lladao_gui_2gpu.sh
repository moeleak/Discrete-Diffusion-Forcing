#!/usr/bin/env bash
# Launch the LLaDA-o D2F adaptation on two GPUs with global batch size 16.
...
# Do not route tqdm through the scheduler's logging pipe: downstream
# backpressure can block rank 0 inside a progress refresh.  The complete
# stdout/stderr stream and the tqdm progress file both live on shared storage.
...
log_exit() {
  local status=$?
  echo "[$(timestamp)] 2-GPU training launcher exited with status ${status}"
}
trap log_exit EXIT
...
echo "[$(timestamp)] starting 2-GPU D2F training"
