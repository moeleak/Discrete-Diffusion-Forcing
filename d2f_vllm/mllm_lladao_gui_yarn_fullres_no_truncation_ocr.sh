...
BENCHMARK_ROOT="${BENCHMARK_ROOT:-$ROOT/data/mind2web-fullpage-native16k-n100-seed42}"
LIMIT="${LIMIT:-100}"
GPU="${GPU:-0}"
KV_CACHE_CAPACITY="${KV_CACHE_CAPACITY:-32768}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
REVISION="${REVISION:-$(git -C "$REPO" rev-parse --short HEAD)}"
RESULT_ROOT="${RESULT_ROOT:-$ROOT/results/d2f-vllm-yarn128k-fullres-no-truncation-ocr-${REVISION}-n${LIMIT}-${RUN_ID}}"
MODEL_OUTPUT="${MODEL_OUTPUT:-$RESULT_ROOT/model}"
FUSED_OUTPUT="${FUSED_OUTPUT:-$RESULT_ROOT/fused}"
MODEL_LOG="${MODEL_LOG:-$ROOT/logs/d2f-vllm-yarn128k-fullres-no-truncation-model-${REVISION}-n${LIMIT}-${RUN_ID}.log}"
OCR_LOG="${OCR_LOG:-$ROOT/logs/d2f-vllm-yarn128k-fullres-no-truncation-ocr-${REVISION}-n${LIMIT}-${RUN_ID}.log}"

if ! [[ "$LIMIT" =~ ^[1-9][0-9]*$ ]] || (( LIMIT > 100 )); then
  echo "full-resolution YaRN OCR LIMIT must be an integer in [1, 100]" >&2
  exit 2
fi
...
echo "[$(date '+%F %T')] YARN_FULLRES_NO_TRUNCATION_OCR_DONE output=$FUSED_OUTPUT"
